...
export default function SedoriManager() {
  ...
  function upsertProduct(data) {
    ...
    const { cardId, ...productData } = data;
    setProducts(prev => prev.some(p => p.id === data.id) ? prev.map(p => p.id === data.id ? productData : p) : [...prev, productData]);
    if (isNew && cardId && productData.purchasePrice > 0 && productData.currentStock > 0) {
      setExpenses(prev => [...prev, {
        id: uid(), date: todayStr(), category: "仕入れ",
        amount: productData.purchasePrice * productData.currentStock,
        cardId, memo: `${productData.name} 仕入れ（初回登録）`,
      }]);
    }
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#E4E0D4] text-[11px] uppercase tracking-widest text-[#8A8471]">
                    ...
                    <th className="text-right px-4 py-3">仕入値</th><th className="text-right px-4 py-3">在庫数</th><th className="text-right px-4 py-3">発注点</th><th className="px-4 py-3"></th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  {filteredProducts.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-[#8A8471] font-mono text-sm">商品がありません。</td></tr>}
                </tbody>
              </table>
            </Card>
          </div>
...
function ProductForm({ initial, cards, onSave }) {
  ...
  const [f, setF] = useState({
    ...
    reorderPoint: initial.reorderPoint ?? 3, createdAt: initial.createdAt ?? todayStr(), cardId: "",
  });
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(f); }}>
      ...
      {isNew && (
        <Field label="仕入れ時の支払カード(任意・経費に自動記録されます)">
...
function CardForm({ initial, onSave }) {
  const [f, setF] = useState({ id: initial.id ?? uid(), name: initial.name ?? "", limit: initial.limit ?? 0 });
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(f); }}>
